...
| `POST` | `/api/v1/map/routes` | `origin`、`destination`、`mode`；公交另需 `originCity`、`destinationCity` | 获取驾车、步行或公交候选路线 |
...
路线接口返回标准化的 `[]evacuation.Route`。驾车和步行调用高德对应 V5 路径，公交调用 `/v5/direction/transit/integrated`，并要求起终点为数字 citycode；代理会把城市字段交给独立的公交端口，不伪造公交结果。路线来源、请求 ID、响应哈希、分段说明和 WGS84 折线会保留在领域结果中。路线风险区排除与安全排序属于 P4.4，在该阶段完成前不能把路线称为安全路线。
...
高德路线是候选路线，不能代表道路已开放，也不能直接作为灾害预警结论。P4.2 已完成 POI 点位风险区过滤；路线几何过滤与安全排序由后续 P4.4 完成。
