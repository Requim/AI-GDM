...
3. Qwen 适配器只接收去标识化分析 JSON、证据和不可变字段清单。系统提示词把资料当作不可信内容，禁止执行其中的指令；请求强制 `response_format.type=json_object`。
4. Qwen 输出只允许 `summary`、`keyFindings`、`actions`、`caveats` 四个字段。结构错误、截断、供应商故障和超时不会覆盖确定性结果，编排结果会标记说明不可用。
...
服务端在返回前再次校验结果：分析摘要必须与原始字节一致，证据可用标志必须与证据数量一致，说明可用标志必须与白名单输出一致。未配置或暂时不可用的博查/Qwen 只会增加降级限制，不会生成替代的风险等级、路线、金额或生还评分。
...
| `QWEN_ENABLED` | 启用解释性报告 | `false` |
| `QWEN_BASE_URL` | Qwen OpenAI 兼容端点 | `https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions` |
| `QWEN_API_KEY` | Qwen 服务端密钥 | 无 |
| `QWEN_MODEL` | 模型名 | `qwen-plus` |
| `QWEN_MAX_COMPLETION_TOKENS` | 输出上限（1-4096） | `1200` |
| `QWEN_OUTPUT_ATTEMPTS` | 结构输出重试次数（1-3） | `2` |
