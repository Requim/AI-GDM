...
docker run --rm -v "$ROOT:/src:ro" -v "$OUTPUT:/out" -w /src \
  -e "GOPROXY=$GO_PROXY" golang:1.26.7-bookworm \
  sh -c 'go test -c -o /out/gdal.test ./internal/adapters/raster/gdal'

docker run --rm -v "$OUTPUT:/test" "$GDAL_IMAGE" \
  sh -c 'GDAL_LIVE_TEST=1 /test/gdal.test -test.run TestLivePipeline -test.v'
