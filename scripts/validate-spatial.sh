...
sh "$ROOT/scripts/run-validation-container.sh" --network "$NETWORK" -v "$ROOT:/src:ro" -w /src \
  ...
  'go mod verify &&
   go test -race ./internal/domain/spatialanalysis ./internal/application/spatialanalysis -count=20 &&
   go test ./internal/adapters/storage/postgres ./internal/adapters/spatial/postgis -count=1'

printf '%s\n' 'P3.2 空间分析确定性与 PostGIS 事务验证通过'
