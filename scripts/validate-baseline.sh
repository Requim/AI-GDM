...
docker run --rm --network "$NETWORK" -v "$ROOT:/src:ro" -w /src \
  ...
  golang:1.26.7-bookworm sh -c '
    go mod verify &&
    go test -race ./internal/domain/loss ./internal/adapters/storage/postgres -count=20 &&
    go vet ./... &&
    go build ./...
  '

printf '%s\n' 'P5.1 版本化损失基线模型、PostgreSQL 仓储和原子替换验证通过'
