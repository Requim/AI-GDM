...
docker run --rm -v "$ROOT:/src:ro" -w /src \
  -e "GOPROXY=$GO_PROXY" "$GO_IMAGE" sh -c '
    go mod verify &&
    go test -race ./internal/adapters/http/webui ./cmd/server -count=20 &&
    go vet ./... &&
    go build ./...
  '

docker run --rm -v "$ROOT:/src:ro" -w /src "$NODE_IMAGE" \
  node --check internal/adapters/http/webui/static/risk-map.js
