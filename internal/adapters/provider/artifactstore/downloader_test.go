...
func TestDownloaderFetch(t *testing.T) {
	...
	artifact := provenance.Artifact{
		...
		Provenance: provenance.Provenance{
			...
			SourceRevision: `"revision-1"`, DataKind: provenance.DataKindNowcast,
...
func TestDownloaderRejectsChangedSourceRevision(t *testing.T) {
	...
	artifact.Reference, artifact.Provenance.SourceURI = server.URL+"/latest.tif", server.URL+"/latest.tif"
	artifact.Provenance.SourceRevision = `"revision-1"`
...
func TestDownloaderRejectsInvalidSourceRevision(t *testing.T) {
	downloader := NewDownloader(httpclient.New(httpclient.Options{}), New(t.TempDir(), 1024), 1024)
	artifact := fixtureArtifact(time.Now().UTC())
	artifact.Provenance.SourceRevision = "invalid\nrevision"
	_, err := downloader.Fetch(context.Background(), artifact)
